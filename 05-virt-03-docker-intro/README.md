...
[ссылка hub.docker.com](https://hub.docker.com/repository/docker/romangvol1/custom-nginx/general/)
...
![task5.7](https://github.com/RomanVol1/virtd-homeworks/blob/shvirtd-1/jpg/task5.7.jpg)

```
Говорится что найдены потерянные контейнеры  для этого проекта: "task5-portainer-1". Суть предупреждения заключается в том, что Docker Compose обнаружил контейнеры, которые не связаны с сервисами, определенными в  "docker-compose.yaml". Это может произойти если было изменено название или был удалён сервис, но контейнеры все ещё существуют.
Предлагается выполнить команду с флагом "--remove-orphans", чтобы очистить потерянные контейнеры. Это удалит все контейнеры, которые не связаны с определенными сервисами в файле "docker-compose.yaml".
```
